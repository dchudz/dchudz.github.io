...
PlotPointEstimates <- function(predictionData, title) {
  ggplot(predictionData, aes(x=factor(X2), y=Prediction, color=factor(X1), group=X1)) + 
    ...
    geom_text(aes(label="(unseen in train)"), vjust=-1, data=subset(predictionData, Unseen), show_guide  = F) +  
    ggtitle(title) + 
    xlab("X2") +
...
nTrees <- 100000
shrinkageRelativeToDefault <- 1

fitGbmAndPlotPredictions <- function(nTrees, shrinkageRelativeToDefault, ... ) {
  gbmFit <- gbm(Y ~ X1 + X2, 
                data = train,  
                n.trees = nTrees, 
                shrinkage = shrinkageRelativeToDefault*.001,
                ...)
  predictions$GbmPrediction <- predict(gbmFit, predictionFeatures, nTrees)
  
  
  ggplot(predictions, aes(x=factor(X2), y=GbmPrediction, color=factor(X1), group=X1)) + 
    geom_line() +  
    geom_text(aes(label="(previously unseen)"), vjust=-1, data=subset(predictions, Unseen)) +
    ggtitle("GBM Model Predictions: X2 only matters when X1=1") + 
    xlab("X2") +
    scale_color_discrete(name = "X1") +
    geom_point(aes(y=Y, shape="Train"), data=train)
 
}


fitGbmAndPlotPredictions(nTrees, shrinkageRelativeToDefault)


fitGbmAndPlotPredictions(nTrees, shrinkageRelativeToDefault,   interaction.depth = 2)

fitGbmAndPlotPredictions(nTrees, shrinkageRelativeToDefault,   interaction.depth = 2, bag.fraction=.45)

fitGbmAndPlotPredictions(2, 1000)
?gbm
