...
predictions <- transform(predictionFeatures, LmPrediction = predict(lmFit, predictions))
...
When the interaction term is high, `$\beta_2$` makes up for it by being low (and vice versa):
```{r}
...
I won't walk through this situation in detail, but note if we put a prior centered on $0$ on `$\beta_2$` as well as `$\beta_{12}$`, the predictions at `$X_1 = 0$, $X_2 = 1$` would shift to the left. In that case, sets of parameters with a negative interaction term would be penalized twice: once for the negative `$\beta_{12}$`, and again for pushing `$\beta_2$` higher than it otherwise had to be.
...
predict(bartFit, predictions[c("X1", "X2")])

bartPredictions <- bartMachine::bart_machine_get_posterior(bartFit, predictions[c("X1", "X2")]) %>%
  data.frame %>%
  cbind(predictions[c("X1", "X2")]) %>%
...
# 
# bartPredictions <- 
#   bartMachine::bart_machine_get_posterior(bartFit, predictions[c("X1", "X2")])$y_hat_posterior_samples %>%
#   data.frame %>%
...
# 
# preds <- bartMachine::bart_machine_get_posterior(bartFit, predictions[c("X1", "X2")])
# 
